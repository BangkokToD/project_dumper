...
class ScanService:
    ...
    def scan(root: Path, cfg: Config, options: ScanOptions) -> ScanResult:
        ...
        Семантика:
        - В дереве collapsed папка видна, но её содержимое заменяется на "…".
        - В дампе (файлы) содержимое collapsed и вручную исключённые файлы
          не показываются по умолчанию и появляются только при ignore_collapsed.
        - Режимы:
